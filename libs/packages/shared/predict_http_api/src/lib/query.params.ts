...
export class OpenApiHttpParams {
  ...
  constructor(
    ...
    defaults?: { explode?: boolean; delimiter?: Delimiter },
  ) {
  ...
  > {
    ...
    for (const [key, entry] of this.params.entries()) {
      ...
      if (entry.options.explode) {
        parts[encodedKey] = entry.values.map((v) =>
          this.encoder.encodeValue(v),
        );
      } else {
        const encodedValues = entry.values.map((v) =>
          this.encoder.encodeValue(v),
        );
  ...
  toHttpParams(): HttpParams {
    ...
    const httpParams = new HttpParams({
      encoder: new IdentityHttpParameterCodec(),
    });
...
export function concatHttpParamsObject(
  ...
  return httpParams.set(key, keyAndValues, {
    explode: false,
    delimiter: delimiter,
  });
}
